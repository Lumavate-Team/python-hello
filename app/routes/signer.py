...
class Signer:
  ...
  def get_request_signing_errors(self):
    return self.get_signing_errors(request.method.lower(), request.url.lower(), request.get_data(), request.headers)
  ...
  def get_signature(self, method, url, body, headers, forced_time=None, forced_nonce=None):
    ...
    signature = hmac.new(str(self.private_key).encode('utf-8'), msg=key.encode('utf-8'), digestmod=hashlib.sha256).hexdigest()

    additional_query['s-signature'] = signature

    additional_query_string = '&'.join([k + '=' + str(additional_query[k]) for k in additional_query])
    if '?' in url:
